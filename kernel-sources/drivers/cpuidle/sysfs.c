...
struct rh_cpuidle_state_attr {
	struct attribute attr;
	ssize_t (*show)(struct cpuidle_state *, \
				struct rh_cpuidle_state_usage *, char *);
	ssize_t (*store)(struct cpuidle_state *, \
			struct rh_cpuidle_state_usage *, const char *, size_t);
};

#define define_one_state_ro(_name, show) \
static struct cpuidle_state_attr attr_##_name = __ATTR(_name, 0444, show, NULL)

#define define_one_rh_state_ro(_name, show) \
static struct rh_cpuidle_state_attr attr_##_name = __ATTR(_name, 0444, show, NULL)
...
#define kobj_to_rh_state_usage(k) (kobj_to_state_obj(k)->rh_state_usage)
#define attr_to_stateattr(a) container_of(a, struct cpuidle_state_attr, attr)
#define attr_to_rh_stateattr(a) container_of(a, struct rh_cpuidle_state_attr, attr)
				  ...
				  char * buf)
{
	...
	if (cattr->show) {
		...
		if (!ret)
			ret = rh_cattr->show(state, rh_state_usage, buf);
	}
...
static int cpuidle_add_state_sysfs(struct cpuidle_device *device)
{
	...
	for (i = 0; i < drv->state_count; i++) {
		...
		kobj->rh_state_usage = &device->rh_cpuidle_dev.rh_states_usage[i];
		kobj->device = device;
