...
static struct ctl_table sctp_net_table[] = {
	...
	},
	{
		...
		.data		= &init_net.sctp_probe_interval,
		.maxlen		= sizeof(int),
		.mode		= 0644,
				 ...
				 void *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (write && ret == 0) {
		...
		/* Update the value in the control socket */
		lock_sock(sk);
		sctp_sk(sk)->udp_port = htons(net->sctp_udp_port);
		release_sock(sk);
	}
				       ...
				       void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (write)
		...
		tbl.data = &net->sctp_probe_interval;
	...
	if (write && ret == 0) {
		...
		net->sctp_probe_interval = new_value;
	}
