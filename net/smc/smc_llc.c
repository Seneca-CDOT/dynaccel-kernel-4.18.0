				  ...
				  struct smc_llc_qentry *qentry)
{
	...
	if (flow_type != msg_type)
		pr_warn_once("smc: SMC-R lg %*phN net %ld dropped parallel "
			     "LLC msg: msg %d flow %d role %d\n",
			     SMC_LGR_ID_SIZE, &lgr->id,
			     atomic64_read(&lgr->net->net_cookie),
				    ...
				    int time_out, u8 exp_msg)
{
	...
	if (exp_msg && rcv_msg != exp_msg) {
		...
		pr_warn_once("smc: SMC-R lg %*phN net %ld dropped unexpected LLC msg: "
			     "msg %d exp %d flow %d role %d flags %x\n",
			     SMC_LGR_ID_SIZE, &lgr->id, atomic64_read(&lgr->net->net_cookie),
			     rcv_msg, exp_msg,
			     flow->type, lgr->role,
...
{
	pr_warn_ratelimited("smc: SMC-R lg %*phN net %ld LLC protocol violation: "
			    "llc_type %d\n", SMC_LGR_ID_SIZE, &lgr->id,
			    atomic64_read(&lgr->net->net_cookie), type);
...
{
	pr_warn_ratelimited("smc: SMC-R lg %*phN net %ld link added: id %*phN, "
			    "peerid %*phN, ibdev %s, ibport %d\n",
			    SMC_LGR_ID_SIZE, &link->lgr->id,
			    atomic64_read(&link->lgr->net->net_cookie),
...
{
	if (log)
		pr_warn_ratelimited("smc: SMC-R lg %*phN net %ld link removed: id %*phN"
				    ", peerid %*phN, ibdev %s, ibport %d\n",
				    SMC_LGR_ID_SIZE, &link->lgr->id,
				    atomic64_read(&link->lgr->net->net_cookie),
				    SMC_LGR_ID_SIZE, &link->link_uid,
